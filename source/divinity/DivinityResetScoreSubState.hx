...
class DivinityResetScoreSubState extends MusicBeatSubstate
{
	var bg:FlxSprite;
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1)
	{
		...
		if(week > -1) {
			name = DivinityWeekData.weeksLoaded.get(DivinityWeekData.weeksList[week]).weekName;
		}
		...
		if(controls.BACK) {
			...
			if(onYes) {
				if(week == -1) {
					...
					Highscore.resetWeek(DivinityWeekData.weeksList[week], difficulty);
				}
			}
			FlxG.sound.play(Paths.sound('cancelMenu'), 1);
			#if android
